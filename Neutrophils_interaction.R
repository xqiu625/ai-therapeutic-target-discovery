...
dat0 <- readRDS('/bigdata/godziklab/shared/Xinru/302005/datasets_v5/harmony_int/302005_Neutrophils_cluster.rds')
Anno <- read.delim2('/bigdata/godziklab/shared/Xinru/302005/datasets/per_cell_type_annotation.tsv')
Anno <- Anno %>%
  filter(pbmc == "Y") %>%
  filter(Category != "HC_LPS") %>%
  select(Cell_ID, Disease, Disease_group, Death, Category, pbmc, label)
meta <- dat0@meta.data
meta$Cell_ID2 <- row.names(meta)
meta <- merge(meta, Anno, by = "Cell_ID")
row.names(meta) <- meta$Cell_ID2
dat0@meta.data <- meta
saveRDS(dat0, '/bigdata/godziklab/shared/Xinru/302005/datasets_v5/harmony_int/302005_Neutrophils_cluster_Anno.rds')
...
for (i in 1:length(samples)){
  ...
  cell_type <- pt0@meta.data %>% dplyr::select(label)
  ...
  if (dim(platelet_inter)[1] > 0){
    ...
    write.table(
      platelet_inter,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/",
                    samples[i],'_Platelets_Neutrophils_interaction_score.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data2,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/",
                    samples[i],'_Platelets_Neutrophils_interaction_AVG.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data3,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/",
                    samples[i],'_Platelets_Neutrophils_interaction_SUM.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
