  ...
  for (i in 1:length(samples)){
    ...
    write.table(
      ...
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample/", 
                    Categories[j],"_", samples[i],'_platelet_NK_interaction_score.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data2,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample/", 
                    Categories[j],"_", samples[i],'_platelet_NK_interaction_AVG.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data3,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample/", 
                    Categories[j],"_", samples[i],'_platelet_NK_interaction_SUM.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
