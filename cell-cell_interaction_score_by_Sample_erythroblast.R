...
for (i in 1:length(samples)){
  ...
  if (dim(platelet_inter)[1] > 0){
    ...
    write.table(
      platelet_inter,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample_v2/erythroblast/", 
                    samples[i],'_platelet_erythroblast_interaction_score.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data2,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample_v2/erythroblast/", 
                    samples[i],'_platelet_erythroblast_interaction_AVG.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data3,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/22-02/Interaction_score_sample_v2/erythroblast/", 
                    samples[i],'_platelet_erythroblast_interaction_SUM.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
