...
dat0 <- readRDS('/bigdata/godziklab/shared/Xinru/302005/datasets_v5/harmony_int/302005_DC_cluster.rds')
...
dat0@meta.data <- meta
saveRDS(dat0, '/bigdata/godziklab/shared/Xinru/302005/datasets_v5/harmony_int/302005_DC_cluster_Anno.rds')
  ...
  if (dim(platelet_inter)[1] > 0){
    ...
    write.table(
      ...
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/", 
                    samples[i],'_Platelets_DC_interaction_score.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data2,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/", 
                    samples[i],'_Platelets_DC_interaction_AVG.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
    ...
    write.table(
      data3,
      file = paste0("/bigdata/godziklab/shared/Xinru/302005/v5_output/Interaction_score/", 
                    samples[i],'_Platelets_DC_interaction_SUM.txt'),
      sep='\t',
      quote = FALSE,
      row.names = F
    )
